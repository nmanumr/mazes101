...
export interface MoveTypeParamsMap {
  ...
  [Type.CREATE_CELL_GROUP]: { id: number | string, cellIdx: number };
  [Type.MERGE_CELL_GROUP]: { path1Id: number | string, path2Id: number | string };
  [Type.APPEND_CELL_GROUP]: { id: number | string, cellIdx: number, neighbourCell: number };
  [Type.POP_CELL_GROUP]: { id: number | string, cellIdx: number };
}
