import { Direction, RectangularBoard, toPosition } from "../boards/rectangular.ts";
...
export function render<T = string>(board: RectangularBoard, options: Partial<RendererOptions<T>> = {}): T {
    ...
    board.cells.forEach((cell, index) => {
        ...
        let x1o = hasLeftWall ? 0 : -opts.cellGap, x2o = hasRightWall ? 0 : opts.cellGap, y1o = hasTopWall ? 0 : -opts.cellGap, y2o = hasBottomWall ? 0 : opts.cellGap;
        if (hasTopWall) {
    ...
    }), opts.shouldFillPath && h('path', { d: defaultPath, strokeWidth: `0`, fill: colors.default }), h('path', { d: walls, strokeWidth: `${opts.lineWidth}px`, strokeLinecap: 'round' }));
}
